...
Predicted <- data.frame(Year=26,High=28.36308 ) 
Actual <- data.frame(Year=26,High=38)
predicted <- sprintf("%.2f", Predicted$High)
actual <- sprintf("%.2f", Actual$High)
preditxt <- paste("Predicted High Temp of :", predicted)
actutxt <- paste("Actual High Temp of :", actual)
both <- rbind(Predicted,Actual)

ggplot(data=weather,mapping=aes(x=Year,y=High, col=High))+
  geom_point()+
  
  annotate("text", x = 25.5, y = 35, label = "The distance from the purple dot to the red one is 9.64 (farenheit) \n We get that by subtracting purple Y - red Y\n In other words our actual observation - our prediction\n This is how you get the residuals for each observation\n Learn More In The Residuals Section Below", hjust = 1, vjust = 0, color = "black", size=2) +  
  
  geom_segment(data = both, aes(x = 26, y = 28.36308, xend = 26, yend = 38), color = "black") +
  
  geom_point(Predicted, mapping=aes(x=Year, y=High), color="red", size=2.5) +
  geom_point(Actual, mapping=aes(x=Year, y=High), color="purple", size=2.5) +
  
    geom_label_repel(data = Predicted, aes(x = Year, y = High, label = preditxt), color = "red", size = 2, nudge_x = .2, nudge_y = 1.5)+
  geom_label_repel(data = Actual, aes(x = Year, y = High, label = actutxt), color = "purple", size = 2, nudge_x = .2, nudge_y = 1.5)+
...
ggplot(data=weather,mapping=aes(x=Year,y=High, col=High))+
  geom_point()+
  geom_smooth(method = "lm", formula=y~x, se=FALSE, col="black")+
  geom_segment(aes(x=Year, xend=Year, yend=High))+
  geom_rect(aes(xmin=Year, xmax=Year+weatherLm$res*.5, ymin=weatherLm$fit, ymax=High),col="grey50", fill="grey90")+
  labs(
    x="Year from 1998 to 2023",
    y="High temperature in Fahrenheit",
    title="(SSE) All January 22nd High Temperatures for the past 25 years",
    col="Temperature")+
  theme_bw()
```
...
# Calculate mean of y
mean_y <- mean(weather$High)

# Predicted values from the linear regression model
weather$predicted <- predict(weatherLm, newdata = weather)

# Create the base plot with xlim to limit the x-axis
p <- ggplot(weather, aes(x = Year, y = High, col=High)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, col="black") +
  geom_hline(yintercept = mean_y, linetype = "dashed", col="orange") +
  geom_segment(aes(x = Year, y = mean_y, xend = Year, yend = predicted), col="black", linetype = "solid") +
  xlim(0, 26)  # Set the x-axis limit
...
p + theme_bw() +
  labs(
    x = "Year",
    y = "High temperature",
    title="(SSR) All January 22nd High Temperatures for the past 25 years",
    col="Temperature")
...
SSTO <- sum((weather$High - mean_y)^2)


ggplot(weather, aes(x = Year, y = High, col=High)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "grey", linetype="dashed") +
  geom_hline(yintercept = mean_y, linetype = "solid", color = "black") + 
  geom_segment(aes(x = Year, y = High, xend = Year, yend = mean_y), col="black", linetype = "dotted")+ 
  labs(
    x = "Year",
    y = "High Temp",
    title="(SSTO) All January 22nd High Temperatures for the past 25 years",
    col="Temperature")+
  theme_bw()
